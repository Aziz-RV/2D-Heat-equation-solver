                                            ...
                                            Eigen::MatrixXd& X)
{
    ...
    */
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        num = X(0,i);
        u_yL = sin(pi*num)*std::sinh(pi); // since u(x,L) = sin(pi*x)*sinh(pi) the values of x-coordinate are needed
                                    ...
                                    std::string ref_filename,std::string solution_filename)
{
   ...
   int x0 = 0;
    ...
    double u_x0 = 0; // x = 0 // u(0,y) = 0 
    double u_xL = 0; // x = L // u(L,y) = 0
    double u_y0 = 0; // y = 0 //u(x,0) = 1
